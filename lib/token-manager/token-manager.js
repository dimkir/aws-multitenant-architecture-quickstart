 ...
 */
module.exports.getCredentialsFromToken = function(req, returnCredentials) {
    winston.debug('getCredentialsFromToken()');
    ...
    if (bearerToken) {
        ...
        if (!(tokenValue in tokenCache)) {
            ...
            async.waterfall([
                ...
                if (error) {
                    winston.error('Error fetching credentials for user: ' + error)
                    returnCredentials(null);
                }
                else {
                    tokenCache[tokenValue] = results;
                    returnCredentials(results);
                }
            });
        }
        else if (tokenValue in tokenCache) {
            winston.debug('Getting credentials from cache');
            returnCredentials(tokenCache[tokenValue]);
        }
    }
    else{
        winston.error(`Cannot find bearer token within request, make sure Authorization header is set`);
        returnCredentials(null);
    }
};
...
function getUserPoolWithParams(userName, callback) {
    winston.debug(`getUserPoolWithParams(${userName}, ...)`);
    ...
    request({
        ...
        if (!error && response.statusCode === 200) {
            ...
            callback("Error loading user: " + error);
        }
    });
}
 ...
 * >DK Should be called getCredentialsForPoolUser()
...
function authenticateUserInPool(userPool, idToken, callback) {
    winston.debug(`authenticateUserInPool(userPool, idToken, ...)`);
    ...
    getIdentity(params, function (ret, data) {
        ...
        if (ret) {
            ...
            getCredentialsForIdentity(params, function (ret, data) {
                if (ret) {
...
function getCredentialsForIdentity(event, callback) {
    winston.debug(`getCredentialsForIdentity(event, ...)`);
 ...
 */
function getIdentity (event, callback) {
    winston.debug(`getIdentity(event, ...)`);
